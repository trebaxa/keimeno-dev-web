<section class="section-soft section-space" id="services">
  <div class="container">
    ...
    <div class="row">
      <div class="col-12 mb-4">
        <ul class="nav nav-pills service-tabs gap-2" id="nav-<%$flxt.var.fv_titel_8|md5%>" role="tablist">
          <% foreach from=$flxt.var.fv_leistungen item=row %>
          <%$tab_id = $row.fv_titel|md5%>
          <li class="nav-item" role="presentation">
            <button class="nav-link<% if ($row@first) %> active<%/if%>" id="tab-<%$tab_id%>tab" data-bs-toggle="tab" data-bs-target="#content-<%$tab_id%>" type="button" role="tab" aria-controls="content-<%$tab_id%>" aria-selected="<%if ($row@first)%>true<%else%>false<%/if%>">
              <%$row.fv_titel%>
            </button>
          </li>
          <%/foreach%>
              ...
              <div class="row g-4 align-items-center">
                <div class="col-12 col-md-6 col-lg-4">
                  <div class="service-illustration rounded-4 p-3">
                    <img class="img-fluid rounded-4" src="<%$row.fv_themenbild_img_md%>" alt="<%$row.fv_themenbild_alt|sthsc%>" loading="lazy">
                  </div>
                </div>
