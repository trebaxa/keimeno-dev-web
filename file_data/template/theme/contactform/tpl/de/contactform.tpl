  ...
  <div class="container">
    ...
    <div class="row g-4 align-items-start">
      <div class="col-12 col-lg-8">
        <form action="<%$THISURL%>" method="post" enctype="multipart/form-data" id="js-contact-<%$cont_matrix_id%>" data-event="contactform" class="fmrcontactform jsonform contact-card contact-form p-4 p-lg-5" role="form">
          ...
          <div class="row">
            <div class="col-12 col-md-6">
              ...
              <div class="form-group">
                <label for="contact-elist-<%$cont_matrix_id%>">Empf&auml;nger*</label>
            ...
            <div class="col-12 col-md-6">
              <div class="form-group">
                <label for="contact-email-<%$cont_matrix_id%>">E-Mail*</label>
                <input data-event="check-email" type="email" class="required form-control<% if ($kregform_err.tschapura!='') %> has-error<% /if %>" id="contact-email-<%$cont_matrix_id%>" placeholder="E-Mail" required name="FORM[tschapura]" value="<% if ($CU_LOGGEDIN==true) %><%$customer.email%><%else%><% $CONTACTF.values.tschapura|sthsc %><%/if%>" aria-required="true">
                <input type="hidden" name="FORM_LABEL[email]" value="E-Mail">
              </div>
              <div class="form-group">
                <label for="contact-phone-<%$cont_matrix_id%>">R&uuml;ckrufnummer*</label>
                <input type="text" class="form-control<% if ($kregform_err.tel!='') %> has-error<% /if %>" id="contact-phone-<%$cont_matrix_id%>" placeholder="Ihre R&uuml;ckrufnummer" required name="FORM[tel]" value="<% $CONTACTF.values.tel|sthsc %>" aria-required="true">
          ...
          <div class="contact-upload-group my-4" aria-labelledby="contact-upload-heading-<%$cont_matrix_id%>">
            <h3 class="h5 mb-2" id="contact-upload-heading-<%$cont_matrix_id%>">Dateien anh&auml;ngen</h3>
            <p class="small text-secondary mb-3">F&uuml;gen Sie relevante Dokumente oder Bilder zur Anfrage hinzu.</p>
            <%for $i = 1 to $contact.cf_fileuploads%>
            <div class="upload-field mb-3">
              <label class="form-label" for="contact-file-<%$cont_matrix_id%>-<%$i%>">
                <i class="fa-solid fa-paperclip me-2" aria-hidden="true"></i>Datei ausw&auml;hlen
              </label>
              ...
              <div class="form-text" id="contact-file-help-<%$cont_matrix_id%>-<%$i%>">Mehrfachauswahl ist m&ouml;glich.</div>
          ...
          <% if ($contact.cf_captcha>=1) %>
            ...
            <fieldset class="form-group" aria-describedby="contact-secode-help-<%$cont_matrix_id%>">
              ...
              <p class="mb-2" id="contact-secode-question-<%$cont_matrix_id%>">
                Aufgabe: <strong><%$contact.captcha_question|hsc%></strong>
              </p>
              <p class="text-secondary small mb-2" id="contact-secode-help-<%$cont_matrix_id%>">
                Bitte l&ouml;sen Sie die Rechenaufgabe und tragen Sie nur das Ergebnis ein.
              </p>
              <%else%>
              <div class="mb-2">
                <img title="{LBL_SECODE}" alt="Sicherheitscode als Bild" src="<%$contact.captcha%>" style="height: 2.3rem;" loading="lazy">
              </div>
              <p class="text-secondary small mb-2" id="contact-secode-help-<%$cont_matrix_id%>">
                Bitte geben Sie den Sicherheitscode aus dem Bild ein.
              </p>
              <%/if%>
            ...
            <div class="form-group">
              <p class="text-secondary small" id="recaptcha-help-<%$cont_matrix_id%>">
                Bitte best&auml;tigen Sie die Sicherheitspr&uuml;fung.
              </p>
      ...
      <div class="col-12 col-lg-4">
        <aside class="contact-card contact-aside p-4" aria-label="Kontaktinformationen">
          <h3>B&uuml;rozeiten</h3>
          <p>Montags bis Freitags 09:00 - 18:00 Uhr.</p>
          <h3>Anschrift</h3>
          <p>
          ...
          <p>
            Telefon:
            <a href="tel:<% $gbl_config.adr_telefon %>" title="Telefonnummer <% $gbl_config.adr_telefon %>"><% $gbl_config.adr_telefon %></a><br>
            Fax: <% $gbl_config.adr_fax %><br>
            E-Mail:
            <a title="Kontakt" href="mailto:<% $gbl_config.adr_service_email %>"><% $gbl_config.adr_service_email %></a>
          </p>
        </aside>
      </div>
    </div>
